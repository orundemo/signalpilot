...
`signalpilot` — first-class TypeScript CLI for the SignalPilot control
...
This package is internal to the `signalpilot` monorepo and is not
...
```
signalpilot login    [--api-url=URL] [--token=BEARER]
signalpilot logout
signalpilot whoami
```
...
```
signalpilot org list
signalpilot org use <org-id>
signalpilot org members
...
```
signalpilot org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
signalpilot project create <name> [--idempotency-key=KEY]
signalpilot env create <project-id> <name> [--idempotency-key=KEY]
signalpilot api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
signalpilot webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
signalpilot usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
signalpilot billing summary
signalpilot audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`signalpilot org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `signalpilot login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/signalpilot/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/signalpilot/config.json` (mode 0644, not a secret). Override
both via `SIGNALPILOT_CONFIG_DIR` (used by tests).
