// CLI error helpers. Translate `SignalPilotError` subclasses (from
...
export class MissingOrgContextError extends Error {
  constructor() {
    super("no active organization (run `signalpilot org use <id>`)");
...
export function formatCliError({ err, mode }: FormatErrorInput): FormattedError {
  ...
  if (err instanceof UnauthenticatedError) {
    return formatSdk(4, err, "token rejected — run `signalpilot login` to refresh", mode);
  }
  if (err instanceof SignalPilotError) {
