// `signalpilot webhook deliveries <endpointId> [--limit=N] [--cursor=CURSOR] [--all]`
...
export async function webhookDeliveriesCommand(
  ...
  if (endpointId === undefined || endpointId.length === 0) {
    throw new UsageError(
      "usage: signalpilot webhook deliveries <endpointId> [--limit=N] [--cursor=CURSOR] [--all] [--output=human|json]",
    );
  }
