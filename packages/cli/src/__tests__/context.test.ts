...
describe("resolveConfigDir", () => {
  it("respects SIGNALPILOT_CONFIG_DIR override", () => {
    const prev = process.env["SIGNALPILOT_CONFIG_DIR"];
    process.env["SIGNALPILOT_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["SIGNALPILOT_CONFIG_DIR"];
      else process.env["SIGNALPILOT_CONFIG_DIR"] = prev;
    }
  });
});
