...
SignalPilot TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the SignalPilot control plane API.

## Install (workspace)

This package is internal to the `signalpilot` monorepo. Add it as a
...
const client = new SignalPilot({
  baseUrl: "https://api.signalpilot.orun.new",
  auth: { kind: "bearer", token: process.env["SIGNALPILOT_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `SignalPilotError`. Branch on
...
| `SignalPilotError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `SignalPilotError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new SignalPilot({ fetch })`.
