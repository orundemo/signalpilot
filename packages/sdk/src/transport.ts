// Base HTTP transport for the SignalPilot SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `SignalPilotError`
...
export interface ClientOptions {
  /**
   * Base URL of the SignalPilot api-edge worker, e.g.
   * `https://api.signalpilot.orun.new`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `SignalPilot` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "SignalPilot SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "SignalPilot SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
