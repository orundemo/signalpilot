...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new SignalPilot({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => SignalPilotError]> = [
  ...
  it("forward-compat: unknown error code → base SignalPilotError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(SignalPilotError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as SignalPilotError).code).toBe("future_quota_exceeded");
    }
  });
