...
const ORG_ROLE_PERMISSIONS: Record<OrganizationRole, readonly string[]> = {
  ...
  builder: [
    ...
    "organization.metering.read",
    "organization.integration.read",
    // builder runs the product; only weight tuning is withheld.
    "organization.prospect.read",
    "organization.prospect.write",
    "organization.prospect.archive",
    "organization.discovery.read",
...
const ALL_KNOWN_ACTIONS: ReadonlySet<string> = new Set([
  ...
  "project.repo_link.write",
  // ── prospecting (SP0) ─────────────────────────────────────
  // Registration is mandatory: an unregistered action denies with
  // `unknown_action` before any role lookup happens.
  "organization.prospect.read",
  "organization.prospect.write",
  "organization.prospect.archive",
  "organization.discovery.read",
