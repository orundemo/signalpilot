...
import { isProspectingRoute, handleProspectingRoute } from "./prospecting-facade";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    } else if (isProspectingRoute(url.pathname)) {
      response = await handleProspectingRoute(request, env, requestId, url.pathname);
