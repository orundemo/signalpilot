...
// deleted; all wire I/O now goes through the typed `SignalPilot` resource
...
//   - `createClient` — constructs a `SignalPilot` against a target, wired
...
//     `Promise<T>` + thrown `SignalPilotError` into the same shape the
...
import { SignalPilot, SignalPilotError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = SignalPilot;
...
export function createClient(target: ApiTarget, token: string | null): SignalPilot {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new SignalPilot(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `SignalPilotError` subclasses surface as `{ ok: false }` carrying the
...
export async function wrap<T>(fn: () => Promise<T>): Promise<ApiResult<T>> {
  try {
    ...
    if (e instanceof SignalPilotError) {
