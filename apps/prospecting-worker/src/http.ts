...
/**
 * The typed 402 a spent allowance produces. The details payload carries the
 * meter, the limit, and the reset date so the console can render an upgrade
 * prompt from this response alone — no second round trip.
 */
export function quotaExhausted(
  ...
  return errorResponse("quota_exhausted", message, 402, requestId, { ...details });
}
